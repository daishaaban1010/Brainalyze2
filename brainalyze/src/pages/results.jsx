  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen text-gray-900 flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-6 py-8">
        {/* NO TUMOR */}
        {status === "no_tumor" && (
          <div className="max-w-7xl mx-auto px-12 xl:px-20 py-14">
            <div className="mb-12">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm w-full -mx-2 px-10">
                  ...
                  <div className="mb-8">
                    <div className="flex justify-between items-center text-sm text-gray-600 mb-2">
                      ...
                      <span className="font-semibold text-green-600 bg-green-50 px-4 py-1.5 rounded-full text-sm">High</span>
                    </div>
                    <div className="w-full bg-gray-100 h-3 rounded-full">
                      <div className="bg-green-500 h-3 rounded-full" style={{width: `${resultData?.detectionProbability}%`}} />
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-5">
                    <p className="font-semibold text-gray-900 mb-4 text-base">Analysis Metrics</p>
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2"><span className="text-gray-600 text-base">Sensitivity:</span><span className="font-semibold text-gray-900 text-base">{resultData?.metrics?.sensitivity}%</span></div>
                      <div className="flex items-center gap-2"><span className="text-gray-600 text-base">Specificity:</span><span className="font-semibold text-gray-900 text-base">{resultData?.metrics?.specificity}%</span></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 space-y-8">
                <div className="bg-white p-10 rounded-xl border border-gray-200 shadow-sm relative text-center">
                ...
                <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <textarea
                    ...
                    className="w-full h-40 border border-gray-200 rounded-lg p-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none"
        ...
        {status === "tumor" && (
          <div className="max-w-6xl mx-auto">
            <div className="mb-10">
              ...
              <p className="text-gray-500 mt-2">Comprehensive brain scan analysis completed</p>
            </div>
            <div className="bg-white p-10 rounded-2xl shadow-sm border border-gray-200 text-center relative mb-10">
              <span className="absolute top-6 right-6 bg-red-100 text-red-600 text-sm px-4 py-1 rounded-full font-medium">High Priority</span>
              <div className="w-24 h-24 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6">
                <span className="text-4xl text-red-600 font-bold">!</span>
              </div>
              <h2 className="text-2xl font-bold text-red-600 mb-3">Tumor Detected</h2>
              <p className="text-lg font-medium">Tumor Type: <span className="font-bold ml-2">{resultData?.tumorType}</span></p>
              <p className="text-gray-700 mt-2">Classification: <span className="font-semibold ml-2 text-orange-600">{resultData?.tumorClassification}</span></p>
              <div className="bg-red-100 text-red-600 mt-6 px-6 py-4 rounded-xl text-sm font-medium">
              ...
              <div className="grid md:grid-cols-2 gap-8 mt-10">
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
                  ...
                  <div className="space-y-4 text-gray-700">
                    <div className="flex justify-between"><span>Name:</span><span className="font-medium">{resultData?.patient?.name}</span></div>
                    <div className="flex justify-between"><span>Patient ID:</span><span className="font-medium">{resultData?.patient?.patientId}</span></div>
                    <div className="flex justify-between"><span>Scan Date:</span><span className="font-medium">{resultData?.patient?.scanDate}</span></div>
                    <div className="flex justify-between"><span>Scan Type:</span><span className="font-medium">{resultData?.patient?.scanType}</span></div>
                  </div>
                </div>

                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
                  <h2 className="text-xl font-semibold mb-6">Doctor's Clinical Validation</h2>
                  <textarea
                    value={clinicalNotes}
                    onChange={(e) => setClinicalNotes(e.target.value)}
                    className="w-full h-40 border border-gray-200 rounded-xl p-4 focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                    placeholder="Enter clinical observations..."
                  />
                  <div className="flex justify-end gap-4 mt-8">
                    <button className="px-6 py-3 border rounded-xl font-medium hover:bg-gray-50">Export as PDF</button>
                    <button className="px-6 py-3 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700">Save & Finalize Report</button>
                  </div>
                </div>
              </div>
            </div>
